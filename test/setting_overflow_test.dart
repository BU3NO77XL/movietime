...
import 'package:appflutter/screens/setting.dart';
...
void main() {
  ...
  for (final (label, w, h) in sizes) {
    testWidgets('Setting no overflow at $label', (tester) async {
      final errors = await pumpAndCollect(tester, w, h);
      expect(
        errors,
        isEmpty,
        reason: 'Setting overflow at $label\n${errors.join('\n')}',
      );
    });
  }

  testWidgets('Setting quality row opens video quality page', (tester) async {
    ...
    expect(find.byType(VideoQualityScreen), findsOneWidget);
