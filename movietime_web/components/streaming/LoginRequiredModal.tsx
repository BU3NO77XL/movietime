            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-md rounded-xl shadow-[0_28px_80px_rgba(0,0,0,0.85)] overflow-hidden z-10"
                    ...
                    >
                        ...
                        <div className="relative z-20 -mt-2 px-7 pb-6 flex flex-col items-center text-center">
                            ...
                            {/* Lista de Benefícios — tipografia maior para legibilidade */}
                            <motion.div
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ delay: 0.22, duration: 0.38 }}
                                className="w-full bg-white/[0.04] border border-white/10 rounded-lg p-4 sm:p-5 mb-6 text-left space-y-4"
                            >
                                {BENEFITS.map((item, index) => {
                                    const Icon = item.icon;
                                    return (
                                        <div key={index} className="flex items-start gap-3.5">
                                            <div className="p-2 rounded-md bg-white/10 text-white shrink-0 mt-0.5">
                                                <Icon className="w-5 h-5" strokeWidth={2.5} />
                                            </div>
                                            <div className="min-w-0">
                                                <h4
                                                    className="text-[15px] sm:text-base font-bold text-white leading-snug"
                                                    style={{ fontFamily: '"Netflix Sans"' }}
                                                >
                                                    {item.title}
                                                </h4>
                                                <p
                                                    className="text-[13px] sm:text-sm text-white/70 leading-snug mt-1"
                                                    style={{ fontFamily: '"Netflix Sans"' }}
                                                >
                                                    {item.desc}
                                                </p>
